...
If the user cannot be identified from the call because they have not authenticated, the service must respond with HTTP 403 (Forbidden).  The response message should indicate that authentication is required to use the service.

If an authenticated user could not be identitfied from the HTTP request the service must response with HTTP 400 (Bad Request).  The response message should indicate that the user is unknown to the system.
