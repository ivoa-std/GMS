...
The service resolves the Group URI to the URL hosting the associated GMS search capability via a registry lookup:
...
\begin{verbatim}
HTTP GET to https://server.example.com/gmsService/search?group=my-collaboration
\end{verbatim}
...
When a user calls this protected service, the service needs to authorize use by making the following call to the associated GMS.

\begin{verbatim}
HTTP GET to
https://server.example.com/gmsService/search?group=project-group-1&group=project-group-2
\end{verbatim}
...
In anticipation of more rows to follow, and to avoid needing to make multiple calls to GMS, the TAP service asks for all the user's groups when the first protected row is encountered.  This cached group information can be applied to all subsequent rows processed.  Again, the service uses the end user's delegated credentials in the search call.
