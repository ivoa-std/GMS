...
An alternative to this approach, which was thoroughly considered, is to also allow the use of privileged credentials to make GMS calls.  That is: allow the set of systems that need to do authorization checks to make GMS calls for any target user.  There are a number of problems with this.
...
Another problem with making external GMS calls with privileged accounts is the need for a trust arrangement between the hosts.  Organizations would need to ask each other to allow GMS calls to work for certain privileged accounts.
...
\item By using LDAP only with group membership plugins.
